  ...
  return (
    <div className="bg-[#FDF8F0] text-[#2C2A24] font-sans w-[8.5in] min-h-[11in] shadow-xl print:shadow-none p-[0.75in] flex flex-col border border-[#D4C8B8] print:border-none mx-auto">
      <div className="border-b border-[#D4C8B8] pb-6 mb-6 flex justify-between items-end">
        ...
        <div className="text-[8px] font-light uppercase tracking-[0.3em] text-[var(--theme-color)] text-right">
      ...
      {data.summary && (
        <p className="text-[13px] leading-relaxed text-[#4A3A2A] border-l-2 border-[var(--theme-color)] pl-4 mb-8 italic">
          {data.summary}
        </p>
      )}
      ...
      {data.experience.length > 0 && (
        <div className="mb-8">
          <h2 className="text-[8px] font-light text-[var(--theme-color)] uppercase tracking-[0.4em] mb-5">experience</h2>
          <div className="space-y-6">
            {data.experience.map(exp => (
              <div key={exp.id}>
                <div className="flex justify-between items-baseline">
                  <h3 className="text-md font-light text-[#2C2A24]">{exp.role}</h3>
                  <span className="text-[8px] font-light uppercase tracking-[0.2em] text-[var(--theme-color)]">{exp.startDate} – {exp.endDate}</span>
                </div>
                <p className="text-[9px] font-light uppercase tracking-[0.15em] text-[var(--theme-color)] mb-2">{exp.company}</p>
      ...
      <div className="grid grid-cols-2 gap-12 border-t border-[#D4C8B8] pt-8">
        {data.education.length > 0 && (
          <div>
            <h2 className="text-[8px] font-light text-[var(--theme-color)] uppercase tracking-[0.4em] mb-4">education</h2>
            {data.education.map(edu => (
              <div key={edu.id} className="mb-3">
                <p className="text-md font-light text-[#2C2A24]">{edu.degree}</p>
                <p className="text-[9px] text-[var(--theme-color)] font-light">{edu.school}, {edu.graduationYear}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {data.skills.length > 0 && (
          <div>
            <h2 className="text-[8px] font-light text-[var(--theme-color)] uppercase tracking-[0.4em] mb-4">skills</h2>
      ...
      {data.customSections && data.customSections.length > 0 && data.customSections.map(section => (
        section.items.length > 0 && (
          <div key={section.id} className="mt-8 border-t border-[#D4C8B8] pt-8">
            <h2 className="text-[8px] font-light text-[var(--theme-color)] uppercase tracking-[0.4em] mb-4">{section.title}</h2>
            <div className="space-y-6">
              {section.items.map(item => (
                <div key={item.id} className="pl-6 border-l border-[#D4C8B8]">
                  <div className="flex justify-between items-baseline mb-2">
                    ...
                    {item.date && <span className="text-[9px] font-light text-[var(--theme-color)] tracking-wider">{item.date}</span>}
                  </div>
