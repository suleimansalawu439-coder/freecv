  ...
  return (
    <div className="mb-5">
      <h2
        ...
        style={{ color: DEEP_GRAY }}
      >
        {children}
      </h2>
      <div className="w-10 h-1 mt-2" style={{ backgroundColor: 'var(--theme-color)' }} />
    </div>
  );
}
    ...
    <div className="w-[8.5in] min-w-[8.5in] min-h-[11in] bg-white mx-auto p-[0.85in] font-sans text-gray-900">
      ...
      <header>
        {data.personalInfo.fullName && (
          <h1 className="text-4xl font-black tracking-tight leading-tight" style={{ color: DEEP_GRAY }}>
            {data.personalInfo.fullName}
          </h1>
        )}
        {data.personalInfo.jobTitle && (
          <p className="mt-1 text-base font-medium" style={{ color: 'var(--theme-color)' }}>
            {data.personalInfo.jobTitle}
          </p>
        )}
        {contact.length > 0 && (
        ...
        <div className="mt-6 h-[3px] w-full" style={{ backgroundColor: 'var(--theme-color)' }} />
      </header>
              ...
              <div key={exp.id}>
                <div className="flex justify-between items-baseline">
                  <h3 className="text-sm text-gray-900">
                    ...
                    {exp.role && exp.company && <span className="font-normal text-gray-500"> — </span>}
        ...
        <section className="mt-8">
          ...
          <div className="flex flex-wrap gap-2">
            {data.skills.map(skill => (
              <span key={skill.id} className="border border-gray-300 rounded-full px-3 py-1 text-sm text-gray-700">
                {skill.name}
              </span>
            ))}
          </div>
