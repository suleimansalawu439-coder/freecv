...
export default function Syntax({ data }: { data: ResumeData }) {
  ...
  return (
    <div className="w-[8.5in] min-w-[8.5in] min-h-[11in] bg-white font-sans text-gray-900 px-[0.9in] py-[0.8in] mx-auto">
      {/* Header — name with theme period accent */}
      <header className="mb-8">
        ...
        {info.jobTitle && (
          <p className="text-base text-gray-600 mb-3">{info.jobTitle}</p>
        )}
          ...
          <div className="space-y-5">
            {data.experience.map((exp) => (
              <div key={exp.id}>
                ...
                {exp.description && (
                  <ul className="space-y-1 pl-4 list-disc marker:text-gray-400">
                    {exp.description
                      .split(/\n|\r?\n/)
                      .filter((l) => l.trim())
                      .map((line, i) => (
        ...
        <section className="mb-7">
          ...
          <div className="flex flex-wrap gap-2">
            {data.skills.map((skill) => (
              <code
                key={skill.id}
                className="font-mono text-xs bg-gray-100 text-gray-800 px-2 py-1 rounded"
              >
                `{skill.name}`
              </code>
            ))}
