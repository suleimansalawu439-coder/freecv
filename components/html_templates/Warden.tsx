...
function SectionHeader({ title }: { title: string }) {
  return (
    <h2 className="inline-block border border-slate-400 px-4 py-1.5 uppercase text-xs font-bold tracking-widest text-slate-700 mb-4">
      {title}
    </h2>
  );
}
      ...
      <header className="flex justify-between items-start gap-8 mb-10">
        <div>
          {data.personalInfo.fullName && (
            <h1 className="text-4xl font-bold text-slate-800 leading-tight">{data.personalInfo.fullName}</h1>
          )}
          {data.personalInfo.jobTitle && (
            <p className="text-lg text-slate-500 mt-1">{data.personalInfo.jobTitle}</p>
          )}
        </div>
        {hasContact && (
          <div className="border-2 border-slate-300 rounded-lg p-4 text-sm text-slate-700 space-y-1.5 shrink-0">
        ...
        <section className="mb-8">
          ...
          {data.experience.map((exp) => (
            <div key={exp.id} className="mb-5">
              ...
              {exp.company && <div className="text-sm text-slate-700 mt-0.5">{exp.company}</div>}
              {exp.description && (
        ...
        <section className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {data.skills.map((skill) => (
              <span key={skill.id} className="border border-slate-300 rounded-full px-3 py-1 text-sm text-slate-700">
                {skill.name}
              </span>
            ))}
          </div>
