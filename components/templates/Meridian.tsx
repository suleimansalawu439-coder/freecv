...
export default function Meridian({ data }: { data: ResumeData }) {
  ...
  const darkTheme = darken(themeColor, 0.62);
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
          ...
          {data.showCertifications && data.certifications && data.certifications.length > 0 ? (
            <View style={styles.section}>
              ...
              {data.certifications.map((c) => (
                <View key={c.id} style={styles.expItem}>
                  <View style={styles.expHeader}>
                    <Text style={styles.roleText}>{c.name}</Text>
                    {c.date ? <Text style={styles.dateText}>{c.date}</Text> : null}
                  </View>
                  {c.issuer ? <Text style={styles.bulletText}>{c.issuer}</Text> : null}
                </View>
              ))}
            </View>
          ) : null}
        ...
        <View style={[styles.sidebar, { backgroundColor: darkTheme }]}>
          ...
          {data.skills && data.skills.length > 0 ? (
            <View style={styles.sidebarBlock}>
              ...
              {data.skills.map((s, i) => (
                <View key={s.id} style={styles.meterBlock}>
                  <Text style={styles.meterLabel}>{s.name}</Text>
                  <View style={styles.meterTrack}>
                    <View style={[styles.meterFill, { width: `${62 + ((i * 37) % 34)}%` }]} />
                  </View>
                </View>
              ))}
            </View>
            ...
            <View style={styles.sidebarBlock}>
              ...
              {data.references.map((r) => (
                <View key={r.id} style={styles.eduItem}>
                  <Text style={styles.eduDegree}>{r.name}</Text>
                  <Text style={styles.eduSchool}>
                    {r.title}
                    {r.company ? ` · ${r.company}` : ''}
                  </Text>
                  {r.contact ? <Text style={styles.eduYear}>{r.contact}</Text> : null}
                </View>
              ))}
            </View>
          ) : null}
