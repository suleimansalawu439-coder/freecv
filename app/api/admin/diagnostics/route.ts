...
export async function GET() {
  ...
  checks.env = {
    ...
    GEMINI_API_KEYS_COUNT: (() => {
      const csv = (process.env.GEMINI_API_KEYS || '').split(',').map((k) => k.trim()).filter(Boolean).length;
      let numbered = 0;
      for (let i = 1; i <= 12; i++) if (process.env[`GEMINI_API_KEY_${i}`]) numbered++;
      return csv + numbered || (process.env.GEMINI_API_KEY ? 1 : 0);
    })(),
